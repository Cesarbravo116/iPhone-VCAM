...
static NSFileManager *g_fileManager = nil; // Document Management Objects
static UIPasteboard *g_pasteboard = nil; // clipboard object
static BOOL g_canReleaseBuffer = YES; // Whether the buffer can be released currently
static BOOL g_bufferReload = YES; // Whether the video file needs to be refreshed immediately
static AVSampleBufferDisplayLayer *g_previewLayer = nil; // Native Camera Preview
static NSTimeInterval g_refreshPreviewByVideoDataOutputTime = 0; // If there is VideoDataOutput, The preview screen is synchronized with the VideoDataOutput screen, If not it will read the video directly to display
static BOOL g_cameraRunning = NO;
static NSString *g_cameraPosition = @"B"; // B is the rear camera, F is the front camera.
static AVCaptureVideoOrientation g_photoOrientation = AVCaptureVideoOrientationPortrait; // Direction of the video

NSString *g_isMirroredMark = @"/var/mobile/Library/Caches/vcam_is_mirrored_mark";
NSString *g_tempFile = @"/var/mobile/Library/Caches/temp.mov"; // Location of temporary files
...
+ (CMSampleBufferRef _Nullable)getCurrentFrame:(CMSampleBufferRef _Nullable) originSampleBuffer :(BOOL)forceReNew{
    ...
    // Returns null if no video is replaced to use the original data
    if ([g_fileManager fileExistsAtPath:g_tempFile] == NO) return nil;
    if (sampleBuffer != nil && !g_canReleaseBuffer && CMSampleBufferIsValid(sampleBuffer) && forceReNew != YES) return sampleBuffer; // Returns to the previous buffer if the buffer cannot be released
    ...
    if (g_bufferReload) {
        ...
        @try{
            ...
            // kCVPixelFormatType_420YpCbCr8BiPlanarFullRange   : YUV422 For SD video [420v]
            // kCVPixelFormatType_32BGRA : The output is in BGRA format for OpenGL and CoreImage
            ...
            [reader startReading];
            // NSLog(@"This is the initialization read");
        }@catch(NSException *except) {
            NSLog(@"Initialization error reading video :%@", except);
        }
    }
    // NSLog(@"refreshed");
    ...
    if (newsampleBuffer == nil) {
        ...
        if (originSampleBuffer != nil) {
            ...
            if (copyBuffer != nil) {
                ...
                // NSLog(@"Exit message set %@", CMGetAttachment(copyBuffer, (CFStringRef)@"{TIFF}", NULL));
            ...
            // Read directly from the video kCVPixelFormatType_32BGRA 
            sampleBuffer = newsampleBuffer;
        }
    }
...
-(void) startRunning {
	...
	NSLog(@"Starting to use the camera. The preset is %@", [self sessionPreset]);
	%orig;
}
-(void) stopRunning {
    g_cameraRunning = NO;
	NSLog(@"Stopped using the camera");
	%orig;
}
...
// TODO:: block buffer is not finished yet, so it needs to be here
+ (NSData *)jpegStillImageNSDataRepresentation:(CMSampleBufferRef)jpegSampleBuffer{
    ...
    if (newBuffer != nil) {
        ...
        if (@available(iOS 11.0, *)) { // the problem of rotation
            switch(g_photoOrientation){
                case AVCaptureVideoOrientationPortrait:
            ...
            MSHookMessageEx(
                ...
                imp_implementationWithBlock(^(id self, AVCapturePhotoOutput *output, CMSampleBufferRef photoSampleBuffer, CMSampleBufferRef previewPhotoSampleBuffer, AVCaptureResolvedPhotoSettings *resolvedSettings, AVCaptureBracketedStillImageSettings *bracketSettings, NSError *error){
                    ...
                    if (newBuffer != nil) {
                        ...
                        // NSLog(@"newbuffer = %@", newBuffer);
                        // NSLog(@"oldbuffer = %@", photoSampleBuffer);
                        // NSLog(@"oldpreviewPhotoSampleBuffer = %@", previewPhotoSampleBuffer);
                    }
        ...
        if (@available(iOS 11.0, *)){ // iOS 11+
            ...
            MSHookMessageEx(
                ...
                imp_implementationWithBlock(^(id self, AVCapturePhotoOutput *captureOutput, AVCapturePhoto *photo, NSError *error){
                    ...
                    // There is no buffer here, create one temporarily
                    ...
                    if (newBuffer != nil) { // Hook attribute if new replacement data exists
                        ...
                        CIContext *cicontext = [CIContext new]; // The problem of rotation
                        ...
                        // After getting the newbuffer, start hooking the property
                    ...
                    // NSLog(@"natively photographed previewPixelBuffer = %@", photo.previewPixelBuffer );
                    // NSLog(@"natively photographed fileDataRepresentatio = %@", [photo fileDataRepresentation]);
...
- (void)setSampleBufferDelegate:(id<AVCaptureVideoDataOutputSampleBufferDelegate>)sampleBufferDelegate queue:(dispatch_queue_t)sampleBufferCallbackQueue{
    ...
    if ([hooked containsObject:className] == NO) {
        ...
        // NSLog(@"Prepare the hook-->%@ %p", [sampleBufferDelegate class], original_method);
        ...
        MSHookMessageEx(
            ...
            imp_implementationWithBlock(^(id self, AVCaptureOutput *output, CMSampleBufferRef sampleBuffer, AVCaptureConnection *connection){
                // NSLog(@"Please, just show up! 【self = %@】 params = %p", self, original_method);
                g_refreshPreviewByVideoDataOutputTime = ([[NSDate date] timeIntervalSince1970]) * 1000;

                CMSampleBufferRef newBuffer = [GetFrame getCurrentFrame:sampleBuffer :NO];

                // Refresh preview with buffer
                ...
                if (nowTime - oldTime > 3000) { // Refresh in 3 seconds
                    oldTime = nowTime;
                    // camera info
...
//         NSLog(@"Why is it empty? AVCaptureMetadataOutput");
//         return %orig;
//     }
...
//         NSLog(@"establish contact with setMetadataObjectsDelegate");
//         MSHookMessageEx(
...
//                 // NSLog(@"Capture to metadata %@", metadataObjects);
...
static BOOL g_downloadRunning = NO; // Is the download in progress
...
-(void)decreaseVolume {
    ...
    if (g_volume_up_time != 0 && nowtime - g_volume_up_time < 1) {
        ...
        if (str != nil && [str hasPrefix:@"CCVCAM"]) {
            str = [str substringFromIndex:6]; // Intercepts the string after dropping the subscript 3
        ...
        UIAlertAction *download = [UIAlertAction actionWithTitle:@"download video" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
            // Setting the download address
            UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"download video" message:@"Try to use MOV format video \nMP4 can also be, other types have not yet been tested" preferredStyle:UIAlertControllerStyleAlert];
            ...
            UIAlertAction* okAction = [UIAlertAction actionWithTitle:@"recognize" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
                // Responding to an event Getting a text message
                g_downloadAddress = alert.textFields[0].text;
                NSString *resultStr = @"Convenient mode has been changed to download from remote \n\n Need to ensure that it is an accessible video address \n\n There will be a systematic mute alert when it is completed \n\n Download Failure Disable Replacement";
                if ([g_downloadAddress isEqual:@""]) {
                    resultStr = @"Convenience mode has been changed to select from albums";
                }
                ...
                UIAlertAction *ok = [UIAlertAction actionWithTitle:@"realize" style:UIAlertActionStyleDefault handler:nil];
            ...
            UIAlertAction *cancel = [UIAlertAction actionWithTitle:@"abolish" style:UIAlertActionStyleDefault handler:nil];
        ...
        UIAlertAction *cancel = [UIAlertAction actionWithTitle:@"Cancel operation" style:UIAlertActionStyleCancel handler:nil];
        UIAlertAction *showHelp = [UIAlertAction actionWithTitle:@"- View Help -" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
    ...
    // NSLog(@"Reduced volume? %@ %@", [NSProcessInfo processInfo].processName, [NSProcessInfo processInfo].hostName);
    // %orig;
}
%end
...
%dtor{
    ...
    g_cameraRunning = NO;
    NSLog(@"Uninstallation is complete");
}
